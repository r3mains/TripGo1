...
class PWAInstaller {
    ...
    init() {
        ...
        // Force check for browsers that don't fire beforeinstallprompt
        setTimeout(() => {
            if (!this.isInstalled && !this.deferredPrompt) {
                console.log('🔧 Force showing install option for browsers without auto-prompt');
    ...
    trackEngagement() {
        ...
        interactions.forEach(event => {
            document.addEventListener(event, () => {
                ...
                if (this.engagementScore >= this.minEngagementForPrompt && this.deferredPrompt) {
                    this.checkEngagementAndShow();
                }
    ...
    checkEngagementAndShow() {
        if (this.engagementScore >= this.minEngagementForPrompt && this.deferredPrompt && !this.isInstalled) {
            console.log(`📈 Engagement score: ${this.engagementScore}, showing install prompt`);
    ...
    showSuccessMessage() {
        if (window.tripApp && typeof window.tripApp.showMessage === 'function') {
            window.tripApp.showMessage('🎉 TripGo installed successfully! Background tracking is now enabled.', 'success');
        }
    }
}
